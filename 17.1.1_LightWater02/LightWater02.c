...
int main(void)
{
    ...
    while (1)
    {
      ...
      for (i = 0; i < 8; i++)
      {
        ...
        digitalWrite(latchPin, HIGH);               // Output high level to latchPin,
                                                    // and 74HC595 will update the data to the
                                                    // parallel output port.
        x <<= 1; // make the variable move one bit to left once,
                 // then the bright LED move one step to the left once.
        delay(100);
      }
